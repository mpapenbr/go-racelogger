...
.PHONY: install
## `install`: Fetch dependencies needed to run `go-racelogger`
install:
...
.PHONY: run
## :
## `run`: Run `go-racelogger` in production mode
...
.PHONY: run-debug
## `run-debug`: Run `go-racelogger` in debug mode
